...
then
    ...
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpstreamoptions ssl allow_self_signed --value=true
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpstreamoptions ssl verify_peer --value=false
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpstreamoptions ssl verify_peer_name --value=false
else
    SELF_SIGNED_CERT=no
fi
...
# SMTP mode
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpmode --value="smtp"
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_sendmailmode --value="smtp"

# Encryption
if [ "$PROTOCOL" = "SSL" ]
then
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpsecure --value="ssl"
elif [ "$PROTOCOL" = "STARTTLS" ]
then
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpsecure --value="tls"
elif [ "$PROTOCOL" = "NO-ENCRYPTION" ]
then
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:delete mail_smtpsecure
fi

# Authentification
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpauthtype --value="LOGIN"
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpauth --type=integer --value=1
if [ -n "$SMTP2GO" ]
then
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_from_address --value="no-reply"
else
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_from_address --value="${MAIL_USERNAME%%@*}"
fi
if [ -n "$SMTP2GO" ]
then
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_domain --value="nextcloudvm.com"
else
    sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_domain --value="${MAIL_USERNAME##*@}"
fi
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtphost --value="$MAIL_SERVER"
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpport --value="$SMTP_PORT"
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtpname --value="$MAIL_USERNAME"
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ config:system:set mail_smtppassword --value="$MAIL_PASSWORD"
...
NC_USERS_NEW=$(sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ user:list | sed 's|^  - ||g' | sed 's|:.*||')
mapfile -t NC_USERS_NEW <<< "$NC_USERS_NEW"
for user in "${NC_USERS_NEW[@]}"
do
    if sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ user:info "$user" | cut -d "-" -f2 | grep -x -q " admin"
    then
        args+=("$user" "")
    fi
done
...
sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ user:setting "$choice" settings email "$RECIPIENT"
